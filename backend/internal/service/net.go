...
type ConnectPacket struct {
	...
	Mobile string `json:"mobile"` // ✅ add mobile field
}
...
func (c *NetService) OnIncomingMessage(con *websocket.Conn, mt int, msg []byte) {
	...
	case *ConnectPacket:
		...
		game.OnPlayerJoin(data.Name,data.Mobile, con)
		break
		}
	case *HostGamePacket:
	{
		...
		// 🔥 REMOVE any previous ended game with same quiz ID
		filteredGames := []*Game{}
		for _, g := range c.games {
			if g.Quiz.Id == quiz.Id && g.Ended {
				continue // remove ended game with same quiz
			}
			filteredGames = append(filteredGames, g)
		}
		c.games = filteredGames
