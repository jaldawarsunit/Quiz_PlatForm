...
import { NetService, PacketTypes, type Packet, type ConnectPacket, GameState, type ChangeGameStatePacket, type QuestionAnswerPacket, type PlayerRevealPacket, type ErrorPacket } from "../net";

export const state: Writable<GameState> = writable(GameState.Lobby);
export const points: Writable<number> = writable(0);
export const joinError: Writable<string> = writable("");
...
export class PlayerGame {
    ...
    onPacket(packet: Packet){
        switch(packet.id){
            ...
            case PacketTypes.Error: { // ✅ handle error from server
                let err = packet as ErrorPacket;
                joinError.set(err.message); // ✅ update error store
                break;
            }
        }
    }
